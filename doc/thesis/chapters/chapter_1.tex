...
As we can stated in the previous \autoref{sec:motivation}, enumerating all possible \acrshort{bt} it is not only computationally hard, but in general only partial results are needed.
...
We believe that this implementation is a step forward on the field and we think our approach opens new research lines and improvements to be addressed in the future. 

In order to asses the feasibility of \acrshort{dp} implemented in \acrshort{hs}, we have made as well another contribution with the presentation of a paper in the \acrfull{prole21} Conference~\cite{prole21}. 
...
Finally, and as a result of \acrshort{prole21} work, was the development and publication of a \acrshort{hs} Framework 
