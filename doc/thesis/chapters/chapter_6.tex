...
\begin{definition}[\acrfull{qo}] 
$Q$ is a \emph{\acrfull{qo}} is a value from the  \textit{sum type} %$BT_V + BT_E$ 
$\mathbb{P}(U) + \mathbb{P}(L + E) $
%such that $BT_V \subseteq U\cup L$ and $BT_E \subseteq E$,
producing as a result a set of \acrshort{bt} that include  all the vertices or edges given in $Q$.
\end{definition}
...
Let us illustrate some of the previous definitions with a \acrlong{bg}. 
Lets suppose that we have the following \acrshort{bg} as it is shown in the \autoref{fig:bipartite-graph-example}.

\begin{figure}[ht]
\centering	
\inputtikz{bipartite}
\caption[Example of \acrlong{bg}]{Example of \acrlong{bg}}
...
If we use the previous  definitions, we can detect that \acrshort {bt} in an easier and automated manner.
The first structure that we can detect is \acrlong{awg} as we can see on \autoref{fig:awedge-example}.
...
\end{subfigure}
\caption[Definitions Examples \acrlong{awg}]{ \acrlong{awg} of nodes $a,b,c$ of \autoref{fig:bipartite-graph-example}}
...
\caption[Example Aggregated double-wedge]{\acrlong{adwg} of the pair of nodes $(a,c)$ \\of \autoref{fig:bipartite-graph-example}}
...
\caption[Example Aggregated bi-triangle]{Aggregated bi-triangle of the triple $(a,b,c)$ of \autoref{fig:bipartite-graph-example}}
...
The algorithm first collects \acrshort{awg} for every vertex in the $L$ set of the graph. Afterwards \acrshort{adwg} for every pair of distinct vertices, Finally  constructs \acrshort{abt}  for selected triples of vertices. 
...
$\ab$ downstream the \acrshort{awg} to  it's neighbour $\fbt$ or to $\gbt$ if it is the last $\fbt$. This is because we need to retro feed all the pipeline with all the \acrshort{awg}, in order to find \acrshort{awgc} as we defined in \dref{def:awgc}.
