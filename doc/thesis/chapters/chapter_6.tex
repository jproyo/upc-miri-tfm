...
Then for $\ab$ complexity is $O(3|U|)$ because of intersection and diference operation on Set inside the loop is $O(m+n)$ each. Since $|U| = m = n$ worst case, then the cost of each loop is $O(3|U|)$.  
The complexity of $\ac$ is $O(\binom{|L|}{2} \times 3|U|)$ and it is a little more complex to analize. For building all possible $\dw$ 
...
$\ibt, \obt, gbt$ are not going to be cover because they are straightforward to follow from the code. 
The main part of the algorithmic complexity as we have seen in the pseudo-code relies on actors filter.
...
channels of all the filters. That explain the number of parameters which is generated by the \acrshort{idl}.
...
This chapter presents the details of the \acrshort{dpbt} algorithm. In the first part of the chapter we introduce the algorithm in a succinct manner.
Then, we describe each of the pseudo-code definitions of each component of the algorithm. After that we present a complexity analysis and a correctness proof.
At the end of the chapter we show how we implement this using \acrshort{dpfh} explain in \autoref{dp-hs}.
In the following highlighted line it is the update of the state if we can gather $W_l$ for that $l$.
