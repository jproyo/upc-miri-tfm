...
\caption[\acrshort{awg} on $a$]{This is the \acrshort{awg} $\la a, \{1,2,7\}\ra$, the compact representation of wedges}
...
\caption[\acrshort{bt} $(u_1,l_l,u_2,l_m,u_3,l_u,u_1)$]{A \emph{bitriangle} $(u_1,l_l,u_2,l_m,u_3,l_u,u_1)$ form with \emph{double-wedge} $(u_1,l_l,u_2,l_u,u_3)$ and connector \emph{wedge} $(u_1,l_m,u_3)$}
\end{subfigure}
...
Note that in \autoref{fig:wg-dw-example}, if we find a \emph{wedge} $(u_1,l_m,u_3)$, that "connects" to a \emph{double-wedge} $(u_1,l_l,u_2,l_u,u_3)$ such that $l_m > l_l$ and $l_m < l_u$, then we can build a bitriangle.
In such a case, the \emph{wedge} $(u_1,l_m,u_3)$, is called the \emph{connector wedge}.
...
\caption[{[\acrshort{iebt}] Example Aggregated bitriangle}]{Aggregated bitriangle $\la (a,b,c), \la \{1\},\{1,3,7\},\{1,5\} \ra \ra$ of \autoref{fig:bipartite-graph-example}. 
In this case as the \dref{def:abt} states, $\hat{U}_{(a,b,c)}=\{I,J,K\}$ does not contains disjoint sets, because we are using a compressing mechanism similar as described in \cite{Lai}. 
We can appreciate in the image that we can built an \acrshort{abt} with $b$ which has incidents edges in $1,5$. Note that not all of combinations, for instance form a \acrshort{bt}.}
...
\caption[Actor4 ($actor_4$)]{Actor4 ($actor_4$): Receiving all the Query Commands $Q$ that arrives through Channel $IC_Q$, verify if matches with some Bitriangles that in the state $\ati \subseteq at$ of this Filter. If some bitriangle matches it will be downstream to the Channel $OC_{BT}$ to be processed by the Sink}
...
\caption[Function \mintinline{shell}{buildBtVertex}]{Function \mintinline{shell}{buildBtVertex}: Given a Set of Vertex in $\mathbb{P}(U + L)$, if any of those vertices matches with some vertex in the parameter of the function $\la (l_l, l_m, l_u), \la I,J,K \ra \ra$, build the set of bitriangles that matches that query}
...
\caption[Function \mintinline{shell}{buildBtEdge}]{Function \mintinline{shell}{buildBtEdge}: Given a Set of Edges in $\mathbb{P}(E)$, if any of those edges matches with some edge in the parameter of the function $\la (l_l, l_m, l_u), \la I,J,K \ra \ra$, build the set of bitriangles that matches that query}
