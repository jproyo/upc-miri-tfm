...
A bi-triangle can be described as a sequence $(a,A,b,B,c,C)$  where $\{ a,b,c\} \in L$  and $\{ A,B,C\} \in U$

We need to prove that the algorithm can enumerate all the  bi-triangle in the graph and also that there are not duplicates in the enumeration.

First we are going to prove that if we take all the triples $(a,b,c)$  such $a<b$ and $b<c$ we will show that no bi-triangle $(a,A,b,B,c,C)$ is listed twice.  We will show that we  enumerate all the bi-triangles  where $A,B,C$  are distinct and $A$ is incident to $a$ and $b$, $B$is incident to $a$ and $c$ and $C$ is incident to $b$ and $C$. 

Lets assume that a bi-triangle $(a,A,b,B,c,C)$ such $a<b$ and $b<c$. without loss of generality we  will assume that $a$ small one in the set $\{a,b,c\}$. So we are going to talk from now on of the bi-triangles produced by the filter $F_a$.
 ...
 All the configurations are rejected by filters $F_b$ and $F_c$, and the listing starting with $a$ the ordered one is accepted and the unordered rejected
