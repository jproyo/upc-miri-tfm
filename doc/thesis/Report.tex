...
\PC{$|\dw| \geq 1$ or $\fid$}
\df{\ab{}}{
$\mathtt{(\la l, W_t \ra, \mathbb{DW}, \mathbb{BT}) \leftarrow \gs}$\\
\ForAll{$\la l', W_t' \ra \in IC_{W_1}$}{
      \If{$l' < l$ and $W_t' \cap W_t \neq \emptyset$}{
            $I \leftarrow W_t' \setminus W_t$\\
            $J \leftarrow W_t' \cap W_t$\\
            $K \leftarrow W_t \setminus W_t'$\\
...
\uIf{$\dw = \emptyset$}{$\fd$}
\Else{$\us(\la l, W_t \ra, \mathbb{DW}, \mathbb{BT})$\\}
}
\df{\but{I, J, K}}{
      $U_t' \leftarrow \emptyset$\\
      \ForEach{$i \in I$}{
