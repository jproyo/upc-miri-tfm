...
genSetEdgesConnComp = do 
  ...
  where
    combine :: (S.Set (Edge Integer), ConnectedComponents Integer) -> Integer -> Gen (S.Set (Edge Integer), ConnectedComponents Integer)
    combine (set, cc) _ = do
      edge <- (arbitrary @(Edge Integer)) `suchThat` (\e -> CC.null cc || e `includedIncident` cc)
      return (S.insert edge set, edge `addToConnectedComp` cc)
